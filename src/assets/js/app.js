...
function Confirmation(e, t, n = "POST") {
  ...
  (o.querySelector(".message").innerHTML = e),
    ...
    (o.querySelector("form").querySelector('input[name="_method"]').value = n),
...
document.querySelectorAll(".input-password .icon").forEach((e) => {
  ...
  setTimeout(() => {
    document
      ...
      ?.addEventListener("change", function () {
        document.querySelectorAll(".datatable .table-checkbox").forEach((e) => {
  ...
  document.querySelectorAll("select.select-icons").forEach((e) => {
    e.addEventListener("change", function () {
      let t = e.parentElement.querySelector("div.select-icons .current"),
        n = t.innerHTML;
      (t.innerHTML = ""),
...
function setParamExport() {
    const form = document.getElementById("form-export");
    const forms = document.querySelectorAll('.form-export')
    forms.forEach((item)=>{
        item.querySelectorAll(".input_param").forEach((e) => e.remove());
        var param = queryParams();
        for (let [name, value] of param.entries()) {
            item.insertAdjacentHTML(
                "beforeend",
                `<input type='hidden' class='input_param' name='${name}' value='${value}'/>`
            );
        }
    })
    if (form) {
        form.querySelectorAll(".input_param").forEach((e) => e.remove());
        var param = queryParams();
        for (let [name, value] of param.entries()) {
