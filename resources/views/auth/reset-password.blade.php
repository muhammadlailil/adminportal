<x-portal::layout.blank title="{{ __('adminportal.auth.reset_password.title') }}">
            ...
            <div class="text-card-foreground shaxdow sm:p-6">
                <div class="flex flex-col space-y-2 text-left">
                    ...
                    <p class="text-sm text-muted-foreground">
                        {{ __('adminportal.auth.reset_password.description') }}
                    </p>
                </div>
                <div class="grid gap-6 mt-3">
                    <x-portal::form
                        action="{{ route('admin.auth.reset-password.attempt') }}"
                        method="POST">
                        @csrf
                        <input type="hidden" name="token" value="{{$token}}">
                        <x-portal::form.input type="email" name="email" label="{{ __('adminportal.form.email') }}"
                            required placeholder="name@example.com" value="{{ $email }}" readonly />
